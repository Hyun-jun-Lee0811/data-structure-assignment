...
괄호 표기법으로 입력된 이진트리를 포인터 연결 방식으로 구성한 후, 이진트리의 **전위 순회(Preorder), 중위 순회(Inorder), 후위 순회(Postorder)**를 구현하는 프로그램을 작성하였다.

이번 과제에서는 세 가지 순회 알고리즘을 **재귀 함수로 구현하지 않고**, 반복문과 스택을 이용하여 구현하였다.
